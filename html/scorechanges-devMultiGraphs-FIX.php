...
<?php
	if(empty($_GET["series"]))
		die("error: no series specified.");

	// GET LABELS
	// get start game
	$whereInstance = "1=0";
	foreach ($_GET["series"] as $series) {
	    foreach ($_GET[$series] as $instance)
	    	$whereInstance .= " OR InstanceID = '" . mysql_real_escape_string($instance) . "' ";
	}
	$query = " SELECT MIN(GameNumber) AS sg, MAX(GameNumber) AS eg FROM ScoreChangeCount WHERE (" . $whereInstance . ") 
				AND GameNumber <> 0";
...
var chart = new Chartist.Line('.ct-chart', {
  ...
  series: [
	{ name: 'series-1', data: [<?php echo $series;?>] }
  ]
}, {
  fullWidth: true,
